...
struct ContentView: View {
                ...
                mainTabView
                    .onAppear {
                        if let uid = authService.currentUser?.uid {
                            FirestoreService.shared.startListening(for: uid)
                        }
                    }
                    .onDisappear {
    ...
    private var mainTabView: some View {
        TabView(selection: $selectedTab) {
            ...
            TimeView()
                .tabItem {
                    Label("Schedule", systemImage: selectedTab == 1 ? "calendar" : "calendar")
                }
                .tag(1)
            ...
            NotesView()
                .tabItem {
                    Label("Notes", systemImage: selectedTab == 4 ? "book.fill" : "book")
                }
                .tag(4)
        }
        .tint(DSColor.accent)
        // Frosted glass tab bar — matches the design reference's glass style
    ...
    private func handleDeepLink(_ url: URL) {
        ...
        switch url.host {
        case "tasks": selectedTab = 2
        case "time", "schedule": selectedTab = 1
        case "finance": selectedTab = 3
        case "notes": selectedTab = 4
